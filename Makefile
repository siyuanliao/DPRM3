...
export FFLAGS= -fast -axAVX                           -parallel              -nologo -inline-level=2 -assume byterecl -threads -heap-arrays -I../misc
export FFLAGS= -O3 -axAVX              -funroll-loops -parallel -no-prec-div -nologo -inline-level=2 -assume byterecl -threads -heap-arrays -I../misc
export FFLAGS= -O3 -axAVX              -funroll-loops -parallel -no-prec-div -nologo -inline-level=2 -assume byterecl -threads -heap-arrays -I../misc
...
export FFLAGS= -O3 -fast -parallel -ipo -nostandard-realloc-lhs -nologo -inline-level=2 -assume byterecl -threads -heap-arrays -I../misc -check -traceback
...
export FFLAGS= -mtune=generic -Wall -std=gnu -O3 -I../misc -fopenmp

#compilation and run-time diagnostics on:
# export FFLAGS= -march=native -ffpe-trap=invalid,zero,overflow -Wall -std=gnu -O1 -fcheck=all -fbacktrace -I../misc
...
install:
	(cd misc;	make -k all)
	(cd tslib;	make -k all)
	(cd main;	make -k install)

	@echo " "
	@echo "***************************************"
	@echo "***** DPRM3 installed *****"
	@echo "***************************************"
