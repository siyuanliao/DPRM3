MODULE bdry3Dmod
  ...
  INTEGER            ::  NATIPts( 2 ), NBTYPts( 2 )
         !IsegTopT( 1 ), IsegBotT( 1 ),IsegTopx, IsegTopy,IsegBotx, IsegBoty,
  INTEGER            :: IOStat, IAllocStat, iSmallStepCtr = 0  !ix, iy, 
  !REAL (KIND=8) :: xTopseg( 2 ), yTopseg( 2 ), xBotseg( 2 ), yBotseg( 2 ) 
  !REAL (KIND=8) :: Topx( 3 ),     Botx( 3 )      ! coordinates of corner of active rectangle  
  !REAL (KIND=8) :: Topn( 3 ),     Botn( 3 )      ! tangent and normal    of active triangle   
...
CONTAINS
  ...
  SUBROUTINE GetTopSeg3D( x, IsegTopx, IsegTopy, Topx, Topn, Top_deltax, Top_deltay, &
				xTopseg, yTopseg )
    ...
    REAL (KIND=8) :: Top_tri_n( 2 )

    INTEGER, INTENT(OUT) :: IsegTopx, IsegTopy
    REAL (KIND=8), INTENT( OUT ) :: Topx(3), Topn( 3 )
    REAL (KIND=8), INTENT( OUT ) :: Top_deltax, Top_deltay
    REAL (KIND=8), INTENT( OUT ) :: xTopseg( 2 ), yTopseg( 2 )
  ...
  SUBROUTINE GetBotSeg3D( x, IsegBotx, IsegBoty, Botx, Botn, Bot_deltax, Bot_deltay, xBotseg, yBotseg )
    ...
    INTEGER , INTENT(OUT)       :: IsegBotx, IsegBoty
    REAL (KIND=8), INTENT(OUT)  :: Botx( 3 ), Botn(3)
    REAL (KIND=8), INTENT( OUT ) ::  Bot_deltax, Bot_deltay
    REAL (KIND=8), INTENT( OUT ) ::  xBotseg( 2 ), yBotseg( 2 ) 

    LOGICAL       :: Goodx = .TRUE., Goody = .TRUE.
    INTEGER       :: IsegBotT( 1 )
    REAL (KIND=8) :: Bot_tri_n( 2 )
