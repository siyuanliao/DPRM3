MODULE bdrymod
  ...
  INTEGER            ::  IOStat, IAllocStat, iSmallStepCtr = 0  !ii,
...
CONTAINS
  ...
  SUBROUTINE GetTopSeg( r, IsegTop, rTopSeg )

    ! Get the Top segment info (index and range interval) for range, r

    INTEGER, INTENT(INOUT)      ::IsegTop
    REAL (KIND=8), INTENT(OUT)      :: rTopseg( 2 )
  ...
  SUBROUTINE GetBotSeg( r, IsegBot, rBotseg )

    ! Get the Bottom segment info (index and range interval) for range, r

    INTEGER, INTENT(INOUT)      ::IsegBot
    REAL (KIND=8), INTENT(INOUT)      ::  rBotseg( 2 )
