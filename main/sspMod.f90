MODULE sspmod
  ...
  !INTEGER                :: iSegy = 1  !, iSegr = 1,iSegz = 1,iSegx = 1,
  !INTEGER,       PRIVATE :: iz
...
CONTAINS
  ...
  SUBROUTINE EvaluateSSP( x, c, cimag, gradc, crr, crz, czz, rho, Freq, Task, iSegz,iSegr &
			, iSegx, iSegy )
    ...
    CASE ( 'N' )
       CALL n2Linear( x, c, cimag, gradc, crr, crz, czz, rho, Freq, Task, iSegz )
    CASE ( 'C' )
       CALL cLinear(  x, c, cimag, gradc, crr, crz, czz, rho, Freq, Task, iSegz )
    CASE ( 'P' )
       CALL PCHIP(    x, c, cimag, gradc, crr, crz, czz, rho, Freq, Task, iSegz )
    CASE ( 'S' )
       CALL cCubic(   x, c, cimag, gradc, crr, crz, czz, rho, Freq, Task, iSegz )
    CASE ( 'Q' )
       CALL Quad(     x, c, cimag, gradc, crr, crz, czz, rho, Freq, Task, iSegz, iSegr )
    CASE ( 'H' )
       ...
       CALL Hexahedral( x3, c, cimag, gradc_3d, cxx, cyy, czz, cxy, cxz, cyz, rho, Freq, Task, &
			 iSegz, iSegx, iSegy )
    CASE ( 'A' )
       CALL Analytic( x, c, cimag, gradc, crr, crz, czz, rho, iSegz )
    CASE DEFAULT
...
END SUBROUTINE EvaluateSSP2D
  ...
  SUBROUTINE EvaluateSSP3D( x, c, cimag, gradc, cxx, cyy, czz, cxy, cxz, cyz, rho, Freq, Task, &
    ...
    SELECT CASE ( SSP%Type )
    CASE ( 'N' )
       CALL n2Linear( x_rz, c, cimag, gradc_rz, crr, crz, czz, rho, Freq, Task, iSegz )
    CASE ( 'C' )
       CALL cLinear(  x_rz, c, cimag, gradc_rz, crr, crz, czz, rho, Freq, Task, iSegz )
    CASE ( 'S' )
       CALL cCubic(   x_rz, c, cimag, gradc_rz, crr, crz, czz, rho, Freq, Task, iSegz )
    CASE ( 'H' )
       CALL Hexahedral( x, c, cimag, gradc, cxx, cyy, czz, cxy, cxz, cyz, rho, Freq, Task, &
			iSegz, iSegx, iSegy )
    CASE ( 'A' )
       CALL Analytic3D( x, c, cimag, gradc, cxx, cyy, czz, cxy, cxz, cyz, rho, iSegz )
    CASE DEFAULT
       WRITE( PRTFile, * ) 'Profile option: ', SSP%Type
  ...
  SUBROUTINE n2Linear( x, c, cimag, gradc, crr, crz, czz, rho, Freq, Task, iSegz )
  ...
  SUBROUTINE cLinear( x, c, cimag, gradc, crr, crz, czz, rho, Freq, Task, iSegz)
  ...
  SUBROUTINE PCHIP( x, c, cimag, gradc, crr, crz, czz, rho, Freq, Task, iSegz )
    ...
    CHARACTER (LEN=3), INTENT( IN  ) :: Task
    REAL     (KIND=8), INTENT( OUT ) :: c, cimag, gradc( 2 ), crr, crz, czz, rho ! sound speed and its derivatives
    REAL     (KIND=8) :: h, h1, h2, xt
    COMPLEX  (KIND=8) :: c_cmplx, del1, del2, f1, f2, f1prime, f2prime, fprimeT
    INTEGER,           INTENT(INOUT) :: iSegz
    ...
    IF ( Task == 'INI' ) THEN   ! read in SSP data
       ...
       !  The vector SSP%c0 holds the sound speed value at a node
       !  The vector SSP%c1 holds the estimated first derivative at a node

       ! left endpoint (non-centered 3-point formula, see UCRL-85104)

       CALL h_del( SSP%z, SSP%c, 2, h1, h2, del1, del2 )
       fprimeT = ( ( 2.0D0 * h1 + h2 ) * del1 - h1 * del2 ) / ( h1 + h2 );
       fprimeT = fprime_left_end_Cmplx( del1, del2, fprimeT )

       SSP%c0( 1 ) = SSP%c( 1 )
       SSP%c1( 1 ) = fprimeT

       ! interior nodes (see UCRL-87559)

       DO iSegz = 2, SSP%NPts - 1
          CALL h_del( SSP%z, SSP%c, iSegz, h1, h2, del1, del2 )
          fprimeT = fprime_Cmplx( h1, h2, del1, del2 )

          SSP%c0( iSegz ) = SSP%c( iSegz )
          SSP%c1( iSegz ) = fprimeT
       END DO

       ! right endpoint (non-centered 3-point formula, see UCRL-85104)

       CALL h_del( SSP%z, SSP%c, SSP%Npts - 1, h1, h2, del1, del2 )
       fprimeT = ( -h2 * del1 + ( h1 + 2.0D0 * h2 ) * del2 ) / ( h1 + h2 );
       fprimeT = fprime_right_end_Cmplx( del1, del2, fprimeT )
       
       SSP%c0( SSP%NPts ) = SSP%c( SSP%Npts )
       SSP%c1( SSP%Npts ) = fprimeT

       !                                                               2      3
       ! compute coefficients of the cubic polynomial: c0 + c1*x + c2*x + c3*x
       ! c0 and c1 were done above so just need c2 and c3
       !
       ...
       DO iSegz = 1, SSP%NPts - 1
          ...
          SSP%c2( iSegz ) = ( 3.0D0 * ( f2 - f1 )  - h * ( 2.0D0 * f1prime + f2prime ) ) / h**2
          SSP%c3( iSegz ) = ( h * ( f1prime + f2prime ) - 2.0D0 * ( f2 - f1 ) ) / h**3
       END DO
    ...
    ELSE                        ! return SSP info
       ...
       xt = x( 2 ) - SSP%z( iSegz )
       c_cmplx = SSP%c0( iSegz ) + ( SSP%c1( iSegz ) + ( SSP%c2( iSegz ) + SSP%c3( iSegz ) * xt ) * xt ) * xt

       c     = REAL(  c_cmplx )
       cimag = AIMAG( c_cmplx )

       gradc = [ 0.0D0, REAL( SSP%c1( iSegz ) + ( 2.0D0 * SSP%c2( iSegz ) + 3.0D0 * SSP%c3( iSegz ) * xt ) * xt ) ]

       crr   = 0.0D0
       crz   = 0.0D0
       czz   = REAL( 2.0D0 * SSP%c2( iSegz ) + 6.0D0 * SSP%c3( iSegz ) * xt )
  ...
  SUBROUTINE cCubic( x, c, cimag, gradc, crr, crz, czz, rho, Freq, Task, iSegz )
  ...
  SUBROUTINE Quad( x, c, cimag, gradc, crr, crz, czz, rho, Freq, Task, iSegz, iSegr )
...
!!$     END IF
  ...
  SUBROUTINE Hexahedral( x, c, cimag, gradc, cxx, cyy, czz, cxy, cxz, cyz, rho, Freq, Task, iSegz &
			, iSegx, iSegy )
...
!**********************************************************************!
  ...
  SUBROUTINE Analytic( x, c, cimag, gradc, crr, crz, czz, rho, iSegz )

    REAL (KIND=8), INTENT( IN  ) :: x( 2 )
    REAL (KIND=8), INTENT( OUT ) :: c, cimag, gradc( 2 ), crr, crz, czz, rho
    REAL (KIND=8)                :: c0, cr, cz, DxtDz, xt
    INTEGER,           INTENT(OUT) :: iSegz
...
SUBROUTINE Analytic3D( x, c, cimag, gradc, cxx, cyy, czz, cxy, cxz, cyz, rho, iSegz )
